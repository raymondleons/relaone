...
class Testi extends Component {
  ...
  next() {
    ...
    const nextIndex = this.state.activeIndex === this.props.testimony.length - 1 ? 0 : this.state.activeIndex + 1;
    this.setState({ activeIndex: nextIndex });
  }

  previous() {
    if (this.animating) return;
    const nextIndex = this.state.activeIndex === 0 ? this.props.testimony.length - 1 : this.state.activeIndex - 1;
    this.setState({ activeIndex: nextIndex });
  }
